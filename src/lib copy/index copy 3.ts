...
export class FragmentForm<T extends JSONData = {}> {
	...
	private _commitToSaveValues(values: any) {
		...
		if (equal(this.valuesSavedHistory, this.valuesToSaveHistory)) {
			console.log(structuredClone(this.valuesSavedHistory));
			console.log(structuredClone(this.valuesToSaveHistory));
			console.log('contains -> true', contains(this.valuesSavedHistory, this.valuesToSave));
			this._setSaveStatus(false);
			this.clearAutoSaveDebounce();
			this.valuesToSave = {};
		} //
		else if (this.autoSaveCallback) {
			console.log('contains -> false', contains(this.valuesSavedHistory, this.valuesToSave));
			this._startAutosaveTimer();
...
function contains(target: any, source: any) {
	if (target === source) {
		return true;
	}
	...
	if (targetType === 'boolean' || targetType === 'number') {
